...
GetCstardate <- function(Cstar,Tavg,result.chilling){ ## interpolated version
  ...
  day1=approx(result.chilling[,2],seq(1,nrow(result.chilling)),xout=Cstar, ties="ordered")$y
...
GetPhenDate <- function(critParam,Tavg,actionUnits,datePhen){
  datePhen <- datePhen[[2]]
  #print(c(datePhen, nrow(actionUnits), ncol(actionUnits)))
  accum.Funits <- cumsum(actionUnits[datePhen:nrow(actionUnits),1])
  dayOfYearPred <- which(accum.Funits>=critParam)[1]+datePhen
  #print(list(datePhen, critParam, head(accum.Funits>=critParam), which(accum.Funits>=critParam)[1], head(accum.Funits), tail(accum.Funits)))
  #print(list(datePhen, dayOfYearPred, tail(accum.Funits)))
  return(list(date=Tavg[dayOfYearPred ,3:4],julian=dayOfYearPred))
}
